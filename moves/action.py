...
class SwitchPokemonAction(Action):
    ...
    def perform(self, game_state: GameState) -> bool:
        ...
        if current_pokemon_index != self.from_index:
            game_state.press_key(GameState.DOWN_KEY * (self.from_index - current_pokemon_index))
            game_state.press_key(GameState.A_KEY)

        # Select the second pokemon
        game_state.press_key(GameState.DOWN_KEY * (self.to_index - self.from_index))
        game_state.press_key(GameState.A_KEY)
...
class UseItemAction(Action):
    ...
    def perform(self, game_state: GameState) -> bool:
        ...
        # Select the item
        if not game_state.bag.select_item(bag_index):
            return False
...
class TalkToAction(Action):
    ...
    def perform(self, game_state: GameState) -> bool:
        ...
        return game_state.get_screen() != Screen.OVERWORLD
